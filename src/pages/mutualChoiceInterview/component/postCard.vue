...
<div>
  <div class='postCard' @click="showJobDetail(info.ehireJobId,info.ehireId)">
      ...
      <!-- <p class="item item3"> -->
          <!-- <span class="company">腾讯科技(深圳)有…</span> -->
...
import {getPositionInfoByEhireJobId,getPrd} from '../api/mutualChoiceInterview.js'
export default {
  name:'postCard',
  data() {
    return {
        // label:['定期旅游','按时下班','五险一金']
        show:false,
        jobDetail:{},
        jobId:'',
        ctmId:''
    ...
    showJobDetail(ehireJobId,ehireId){
      ...
      async handleApply(){
        ...
        console.log('this.checkedItem:',this.jobId)
         ...
         if(res.data){     
          //线上地址：
          let openUrl=`https://xym.51job.com/personal/personal_applyjob.aspx?ctmid=${this.ctmId}&jobid=${this.jobId}&prd=xyb_02&prp=&cd=&cp=&ruid=&backurl=${window.location.href}`
          // 开发测试地址
          // let openUrl=`http://sh-dev-176.51job.com/eHirePlusWebApp/htdocs/personal/personal_applyjob.aspx?ctmid=${this.ctmId}&jobid=${this.jobId}&prd=xyb_02&prp=&cd=&cp=&ruid=&backurl=${window.location.href}`
          console.log(openUrl)
